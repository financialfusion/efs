...
public class CustomPPDCreditOrDebit
  extends ACHMapper
  implements ACHClassCode, ACHAccountTypes
{
    ...
    throws FIException
  {
    ...
    String str1 = getStringAttribute(paramErrorMessages, paramRecord, "Individual Name", nameRequired(paramMappingDefinition.getMatchRecordsBy()), localLocale);
    String str2 = getStringAttribute(paramErrorMessages, paramRecord, "Individual Identification Number", idRequired(paramMappingDefinition.getMatchRecordsBy()), localLocale);
    String str3 = getStringAttribute(paramErrorMessages, paramRecord, "Individual Routing Number", accountRequired(paramMappingDefinition.getMatchRecordsBy()), localLocale);
    String str4 = getStringAttribute(paramErrorMessages, paramRecord, "Individual Account Number", accountRequired(paramMappingDefinition.getMatchRecordsBy()), localLocale);
    Integer localInteger = getIntegerACHAccountType(paramErrorMessages, paramRecord, "Individual Account Type", accountRequired(paramMappingDefinition.getMatchRecordsBy()), localLocale);
    String str5 = null;
    if (!isValidTransactionCode(localInteger)) {
      str5 = getCreditDebitAttribute(paramErrorMessages, paramRecord, "Credit or Debit", false, localLocale);
    }
    ...
    if (str5 != null) {
      paramACHEntry.setAmountIsDebit(str5.equals("debit"));
    }
 ...
 * Qualified Name:     com.ffusion.fileimporter.fileadapters.custom.CustomPPDCreditOrDebit
