...
public class ConsumerReportHandler
  extends TransactionTypeSummaryReportHandler
{
  private HashMap jdField_case;
  
  public ConsumerReportHandler(Locale paramLocale)
  {
    super(paramLocale);
  }
  ...
  {
    this.jdField_case = paramHashMap;
    StringBuffer localStringBuffer = new StringBuffer("Select myTemp.countTran, myTemp.tran_type, myTemp.user_id_int, myTemp.sumTran, c.first_name, c.last_name, c.user_name, myTemp.currency_code ");
    localStringBuffer.append("FROM (SELECT COUNT(TRAN_ID) countTran, a.TRAN_TYPE tran_type, a.CURRENCY_CODE currency_code, a.USER_ID_INT user_id_int, SUM(AMOUNT) sumTran FROM AUDIT_LOG a ");
    ...
    localStringBuffer.append(" GROUP BY a.USER_ID_INT, a.TRAN_TYPE, a.CURRENCY_CODE) myTemp, customer c WHERE myTemp.user_id_int=c.directory_id");
    ...
    throws RptException
  {
    ...
    addBlankColumns(paramReportResult, paramLocale, 6);
    localReportColumn = new ReportColumn(paramLocale);
    localArrayList = new ArrayList();
    localArrayList.add(ReportConsts.getColumnName(505, paramLocale));
    ...
    localReportColumn = new ReportColumn(paramLocale);
    localArrayList = new ArrayList();
    localArrayList.add(ReportConsts.getColumnName(506, paramLocale));
    localReportColumn.setLabels(localArrayList);
    localReportColumn.setWidthAsPercent(18);
    paramReportResult.addColumn(localReportColumn);
    localReportColumn = new ReportColumn(paramLocale);
    localArrayList = new ArrayList();
    localArrayList.add(ReportConsts.getColumnName(508, paramLocale));
    localReportColumn.setLabels(localArrayList);
    localReportColumn.setWidthAsPercent(15);
    paramReportResult.addColumn(localReportColumn);
    localReportColumn = new ReportColumn(paramLocale);
    localArrayList = new ArrayList();
    localArrayList.add(ReportConsts.getColumnName(507, paramLocale));
    localReportColumn.setLabels(localArrayList);
    ...
    throws RptException
  {
    ...
    setDimension(localReportResult, 10, getLocale());
    ...
    throws Exception, RptException
  {
    ...
    if (i1 != 0)
    {
      ...
      if (localReportResult != null) {
        k += addBusinessEntriesToReport(localReportResult, localServiceChargeEntries, 6);
      }
      close();
      executeSql(this.jdField_case);
      setPreviousRecord(null);
    }
    return m;
  }
 ...
 * Qualified Name:     com.ffusion.efs.adapters.profile.servicecharge.ConsumerReportHandler
